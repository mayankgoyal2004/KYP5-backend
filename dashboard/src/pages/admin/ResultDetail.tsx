...
export default function ResultDetailPage() {
  ...
  const timeSpentMins = attempt.timeSpent
  ...
  const getSelectedOptionIds = (answer: any) =>
    ...
    answer.selectedOptionIds.length > 0
      ...
      : answer.selectedOptionId
        ? [answer.selectedOptionId]
        : [];
  const isAnswerAttempted = (answer: any) =>
    getSelectedOptionIds(answer).length > 0;
  const totalQuestions =
    attempt.totalQuestions || attempt.test?.totalQuestions || answers.length;
  const correctCount =
    attempt.correctCount ?? answers.filter((a: any) => a.isCorrect).length;
  const wrongCount =
    attempt.wrongCount ??
    answers.filter((a: any) => a.isCorrect === false && isAnswerAttempted(a))
      .length;
              ...
              <div className="flex flex-col items-center md:items-end gap-2 px-6 py-3 bg-muted/20 rounded-xl border">
                ...
                <div className="flex items-center gap-4 text-xs">
                  <span className="flex items-center gap-1">
                    <Target className="h-3 w-3" /> Passing: {passingScore}%
                  </span>
                  <span className="flex items-center gap-1">
                    <Timer className="h-3 w-3" /> Limit:{" "}
          ...
          <TabsContent value="answers" className="space-y-4 py-4">
            {answers.map((answer: any, idx: number) => {
              ...
              return (
                <Card
                  key={answer.id || idx}
                  className={`overflow-hidden border-none ring-1 ${
                    answer.isCorrect
                ...
                >
                  <CardContent className="p-4">
                    <div className="flex items-start gap-4">
                      <div
                        className={`h-10 w-10 shrink-0 rounded-lg flex items-center justify-center font-bold font-mono text-sm ${
                          answer.isCorrect
                            ? "bg-emerald-100 text-emerald-700"
                            : isUnanswered
                        ...
                        <div className="grid sm:grid-cols-2 gap-2">
                          {question.options?.map((opt: any, optIdx: number) => {
                            ...
                            return (
                              <div
                                key={opt.id}
                                className={`flex items-center gap-3 p-3 rounded-xl border ${borderClass} ${bgClass} transition-all`}
                              >
                                <span className="text-xs font-bold text-muted-foreground">
                                  {label}.
                                </span>
                                <span
                                  className={`text-sm flex-1 ${isCorrect ? "font-bold text-emerald-900" : isSelected ? "font-bold text-red-900" : "text-foreground"}`}
                                >
                                  {opt.text}
                                </span>
                                {icon}
