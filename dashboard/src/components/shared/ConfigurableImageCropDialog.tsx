...
export function ConfigurableImageCropDialog({
  open,
  imageSrc,
  fileName,
  mimeType,
  ...
  }, [imageDimensions, outputHeight, outputWidth, zoom]);
  ...
  const clampedOffset = useMemo(() => {
    ...
    const maxY = Math.max(0, (renderedSize.height - outputHeight) / 2);

    return {
      x: clamp(offset.x, -maxX, maxX),
      y: clamp(offset.y, -maxY, maxY),
    };
  }, [offset, outputHeight, outputWidth, renderedSize]);
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const imageLeft = (outputWidth - renderedSize.width) / 2 + clampedOffset.x;
      const imageTop = (outputHeight - renderedSize.height) / 2 + clampedOffset.y;
